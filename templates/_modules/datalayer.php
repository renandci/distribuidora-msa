...
<script>
  <?php
  // Layer Index
  if ('index' == $STORE['dataLayer']['modulo']) { ?>
    dataLayer = [{
  ...
  <?php
  // Layer Produto
  if ('produto' == $STORE['dataLayer']['modulo']) { ?>
    dataLayer = [{
      event: "",
      pageTitle: "<?php echo $STORE['dataLayer']['title'] ?>",
      pageCategory: "<?php echo $STORE['dataLayer']['modulo'] ?>",
  ...
  <?php
  // Layer Produtos
  if ('produtos' == $STORE['dataLayer']['modulo']) { ?>
    dataLayer = [{
      pageTitle: "<?php echo $STORE['dataLayer']['title'] ?>",
      pageCategory: "<?php echo $STORE['dataLayer']['modulo'] ?>",
      event: ""
  ...
  if ('obrigado' == $STORE['dataLayer']['modulo'] || 'finalizado' == $STORE['dataLayer']['modulo']) {
    ...
    if (!empty($pedidos_id)) {
      ...
      $STORE['dataLayer']['total'] = $TOTAL_DATA_LAYER['TOTAL_COMPRA_C_BOLETO'];
  ...
  ?>
    ...
    dataLayer = [{
      'event': 'orderPlaced',
      'transactionId': '<?php echo $STORE['dataLayer']['venda_cod'] ?>',
      'transactionTotal': <?php echo number_format($STORE['dataLayer']['total'], 2, '.', '') ?>,
      'transactionShipping': <?php echo number_format($STORE['dataLayer']['frete_vl'], 2, '.', '') ?>,
      'transactionProducts': <?php echo json_encode($STORE['dataLayer']['produtos'], JSON_UNESCAPED_UNICODE) ?>
    }];
...
$content = $JSqueeze->squeeze(str_replace(['</script>', '<script>'], [null, null], ob_get_clean()), true, false, false);
echo $content;
