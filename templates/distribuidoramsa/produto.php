...
<div id="conteudo-html" class="row">
      ...
      <div class="div-centro-descricao-produto-002">
        <div class="clearfix">
          <div class="clearfix" id="carregar-descricao-texto">
            <div class="row">
              <div class="<?php echo (empty($CONFIG['atacadista']) && (!empty($Produto->id_tamanho) && !empty($Produto->id_cor))) ? 'col-lg-9 col-md-9' : 'col-lg-12 col-md-12' ?> col-sm-12 col-xs-12 mb15">
                ...
                <?php if (in_array('Envio Imediato', explode(',', $Produto->placastatus))) { ?>
                  <div class="row hidden-xs">
                    <div class="col-sm-9 mt10 mb5">
        ...
        <div id="carregar-variacao-produto">
          ...
          <?php if ($ProdutoTamCoun >= 1 && $Produto->id_tamanho > 0) { ?>
            <div class="clearfix border-bottom-dotted mt15 mb15"></div>
            <div class="mostra-carregamento text-center mt20 mb10" style="display:none;"><img src="<?php echo Imgs::src('spinner.gif', 'public') ?>"></div>
        ...
        <div class="clearfix btn-comprar-mobile text-center" id="btn-comprar-mobile">
          <?php if (!empty($STORE['config']['product']['buy']) || !empty($Produto->orcamento)) { ?>
            <div class="text-left solicite-orcamento">
              <div>
                <p>Solicite orçamento para compras pelos meios de comunicação abaixo.</p>
              </div>
              ...
              <div style="cursor:pointer;" onclick="AviseMe.tela(null, true);">
                <span class="fa-stack fa-lg">
  ...
  <div class="hidden-xs" id="recarregar-produtos-relacionado">
    <?php
    ...
    ?>
      ...
      <ul class="produtos-site bg-branco clearfix" id="produtos-relacionado">
        <?php foreach ($Produto->produto_relacionado->grupos_relacionados as $rIndex) { ?>
          ...
          <li id="produto<?php echo $rIndex->id ?>" <?php echo $Produto->id == $rIndex->id ? ' class="hidden"' : '' ?>>
            ...
            <a href="/<?php echo converter_texto($rIndex->nome_produto) ?>/<?php echo $rIndex->id ?>/p" class="cx-lista-produtos" btn-hovers>
              <div class="lista-centro-produtos black-70">
                ...
                <img src="<?php echo $STORE['PRE_LOADED']; ?>" data-original="<?php echo Imgs::src($rIndex->produto_relacao->capa->imagem, 'medium'); ?>" alt="<?php echo $rIndex->nome_produto ?>" class="lazy img-responsive" />
                ...
                <?php
                echo (!empty($STORE['personalize_box_color_top']) ? $STORE['personalize_class']::caixa_cores($rIndex->id) : null);

                echo ($STORE['personalize_class']::{$STORE['personalize_price']}([
                  'preco_venda' => number_format($rIndex->preco_venda, 2, ',', '.'),
                  'preco_size' => 'ft14px',
                  'preco_color' => 'black-40 price-venda',
                ], [
                  'preco_promo' => number_format($rIndex->preco_promo, 2, ',', '.'),
                  'preco_size' => 'ft20px',
                  'preco_color' => 'color-004 price-promo',
                  'preco_boleto' => number_format(desconto_boleto($rIndex->preco_promo, $CONFIG['desconto_boleto']), 2, ',', '.')
                ], [
                  'preco_promo' => $rIndex->preco_promo,
                  'preco_size' => 'ft12px',
                  'preco_color' => 'color-004 price-parcelamento',
                  'preco_parcela_x' => parcelamento($rIndex->preco_promo, $CONFIG['qtde_parcelas'], $CONFIG['parcela_minima']),
                  'preco_parcela_price' => number_format($rIndex->preco_promo / parcelamento($rIndex->preco_promo, $CONFIG['qtde_parcelas'], $CONFIG['parcela_minima']), 2, ',', '.'),
                ]));
                ?>

                <?php if (!empty($STORE['config']['btn_frete']) && CheckFreteGratis($rIndex->preco_promo, $STORE['frete_valor']) || $rIndex->frete > 0) { ?>
                 ...
                 * Botao de Espiar
                 */
                echo !empty($STORE['config']['btn-espiar'][0]) && $rIndex->estoque > 0 ?
                  HelperHtml::button_espiar(
                    '/' . converter_texto($rIndex->nome_produto) . '/' . $rIndex->id . '/p',
                 ...
                 * Botao de Comprar
                 */
                echo !empty($STORE['config']['btn-compra'][0]) && $rIndex->estoque > 0 ?
                ...
                echo (!empty($STORE['personalize_box_color_bottom']) ? $STORE['personalize_class']::caixa_cores($rIndex->id) : null);
                ?>
              </div>
            </a>
          </li>
