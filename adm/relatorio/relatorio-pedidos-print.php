...
// $data_ini = isset($POST['data_ini']) && $POST['data_ini'] != '' ? converterDatas($POST['data_ini']) : null;
// $data_fin = isset($POST['data_fin']) && $POST['data_fin'] != '' ? converterDatas($POST['data_fin']) : null;
...
// $estados = isset($POST['estados']) && $POST['estados'] != '' ? $POST['estados'] : '';
$estados = filter_input(INPUT_POST, 'estados');

// $forma_pagamento = isset($POST['forma_pagamento']) && $POST['forma_pagamento'] != '' ? $POST['forma_pagamento'] : null;
$forma_pagamento = filter_input(INPUT_POST, 'forma_pagamento');

// $MostrarClientes = isset($POST['clientes']) && $POST['clientes'] == 'true' ? true : false;
// $MostrarProdutos = isset($POST['produtos']) && $POST['produtos'] == 'true' ? true : false;

$MostrarClientes = false;
$MostrarProdutos = true;

// $ordem = isset($POST['ordem']) && $POST['ordem'] != '' ? $POST['ordem'] : null;
$ordem = filter_input(INPUT_POST, 'ordem');

$marca = filter_input(INPUT_POST, 'id_marca');

$STATUS = array();

// $STATUS = isset($POST['pedidos']) ? $POST['pedidos'] : null;
...
if ( is_array( $STATUS ) && $STATUS[0] != '') {
    if ($STATUS[0] != '20') {
		$status_array = null;
        foreach ($STATUS as $statusAtivos)
...
$WHERE .= $marca > 0 ? sprintf(' and produtos.id_marca = %u ', $marca) : '';

$ORDER = 'pedidos.data_venda asc ';
$ORDER .= $ordem == 'marca_asc' ? ', marcas.marcas asc ' : '';
$ORDER .= $ordem == 'marca_desc' ? ', marcas.marcas desc ' : '';
...
$pdf->SetTitle('RELATÓRIO DE PEDIDOS', 'UTF8');

$pdf->SetFont('Calibri', '', 13);

$pdf->Cell(200, 5, 'RELATÓRIO DE PEDIDOS', 0, 2, 'C', 0);
...
$pdf->Cell(200, 5, 'Status: ' . ($status_ativos ? $status_ativos : 'Todos'), 'B', 2, 'L', 0);

$pdf->Ln(1);

foreach( $rs as $rws ) 
{
	// if( $order_search_site == 'Estado' && $group_uf != strtoupper($rws['p_uf'])) {
		// $group_uf = strtoupper($rws['p_uf']);
	...
	if( $group_ped != $rws['id_pedido'] ) 
	{
		$group_ped = $rws['id_pedido'];
		...
		$pdf->Cell(20, 5, date('d.m.Y', strtotime($rws['data_venda'])), 0, 0, 'L', 1);
		$pdf->Cell(20, 5, $rws['codigo'], 0, 0, 'L', 1);
		$pdf->Cell(105, 5, '', 0, 0, 'L', 1);
		$pdf->Cell(30, 5, $rws['forma_pagamento'], 0, 0, 'C', 1);
		$pdf->Cell(25, 5, implode(' - ', [$rws['frete_tipo'], 'R$: ' . number_format($rws['frete_valor'], 2, ',', '.') ]), 0, 2, 'C', 1);
		$pdf->Ln(1);			
	...
	if( $MostrarProdutos ) {
		...
		if( $group_mar != $rws['id_marca'] . $rws['id_pedido'] ) {

			$group_mar = $rws['id_marca'] . $rws['id_pedido'];
			$pdf->Ln(1);
			$pdf->setFillColor(102, 102, 102);
			$pdf->Cell(55, 5, '', 0, 0, 'L', 0);
			$pdf->SetFont('Calibri', '', 11);
			$pdf->Cell(145, 5, sprintf('MARCA: %s', $rws['marcas']), 'B', 2, 'L', 1);
			// $pdf->Cell(200, 0, '', 1, 2, 'L', 0);
			$pdf->Ln(0);
		}
		...
		$pdf->SetFont('Calibri', '', 9);
		$pdf->Cell(55, 5, '', 0, 0, 'L', 0);
		$pdf->Cell(110, 5, implode(' - ', [CodProduto($rws['nome_produto'], $rws['id_produto'], $rws['codigo_produto']), $rws['nome_produto'], $rws['marcas']]), 0, 0, 'L', 0);
		$pdf->Cell(10, 5, $rws['quantidade'], 0, 0, 'C', 0);
		$pdf->Cell(25, 5, number_format($rws['valor_pago'], 2, ',', '.'), 0, 2, 'R', 0);
		$pdf->Ln(0);
		...
		// Se tiver Cores
		if( $rws['tipo_a'] ) {
			$pdf->Cell(65, 3, implode(': ', [substr($rws['tipo_a'], 0, 3), null]), 0, 0, 'R', 0);
			$pdf->Cell(135, 3, $rws['nomecor'], 0, 2, 'L', 0);
			$pdf->Ln(0);
		}
		
		// Se tiver Tamanhos
		if( $rws['tipo_b'] ) {
			$pdf->Cell(65, 3, implode(': ', [substr($rws['tipo_b'], 0, 3), null]), 0, 0, 'R', 0);
			$pdf->Cell(135, 3, $rws['nometamanho'], 0, 2, 'L', 0);
			$pdf->Ln(1);
		}
	...
	$sum_all_mercadoria += ($rws['valor_pago'] * $rws['quantidade']);
	
	if( $sum == $rws['total_prod'] ) 
	{	
		$pdf->Ln(1);
		$pdf->SetFont('Calibri', '', 10);
		$pdf->Cell(162, 4, 'Status: ', 0, 0, 'R', 0);
		$pdf->Cell(38, 4, text_status_vendas($rws['status']), 0, 1, 'R', 0);
		...
		$pdf->Cell(38, 4, number_format($rws['desconto_boleto'], 0) . '%', 0, 1, 'R', 0);
		
		$pdf->Cell(162, 4, 'Desconto Cupom: ', 0, 0, 'R', 0);
		$pdf->Cell(38, 4, 'R$: ' .  number_format($rws['TOTAL_CUPOM'], 2, ',', '.'), 0, 1, 'R', 0);
		...
		$sum_all_vlfrete += $rws['frete_valor'];
		$sum_all_desvbol += $rws['desconto_boleto'];
...
if( $tipo_relatorio == 'V' )
{
	...
	foreach( $rs as $cli ) 
	{
		...
		{	
			...
			$pdf->Cell(162, 4, 'Desconto Boleto: ', 0, 0, 'R', 0);
			$pdf->Cell(38, 4, number_format($cli['desconto_boleto'], 0) . '%', 0, 1, 'R', 0);
			
			$pdf->Cell(162, 4, 'Desconto Cupom: ', 0, 0, 'R', 0);
			$pdf->Cell(38, 4, 'R$: ' .  number_format($cli['TOTAL_CUPOM'], 2, ',', '.'), 0, 1, 'R', 0);
