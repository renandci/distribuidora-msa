...
$sql   = ''
  ...
  . 'WHERE p.excluir = 0 ' . PHP_EOL
  . 'GROUP BY p.codigo_id ' . PHP_EOL
  . sprintf(') as P WHERE P.loja_id=%u and P.excluir = 0 ', $CONFIG['loja_id']) . PHP_EOL
...
$ProdutosCount = (int)count(Produtos::find_by_sql($sql));

$total = (ceil($ProdutosCount / $maximo));

$sql .= sprintf("LIMIT %u, %u", $inicio, $maximo);

$Produtos = Produtos::find_by_sql($sql);
...
<script>
  ...
  $("#div-edicao").on("click", "#ml_massa", function(e) {
    ...
    $.ajax({
      ...
      url: "<?php echo $new_uri ?>",
      data: {
        campos: serialisedFormArrayObject,
        mlactive: "true"
      },
      beforeSend: function() {
        $("#status-alteracao").html("Alterando dados em massa, aguarde...").fadeIn(10);
