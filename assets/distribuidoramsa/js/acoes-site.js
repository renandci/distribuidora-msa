...
$(document).ready(function () {
    ...
    if ($(this).val() !== "") {
      $.ajax({
        ...
        success: function (str) {
          $(".retornar-pesquisa").html(str.pesquisa);
        },
  ...
  $("i.icon-lista-menu-mobile").click(function () {
    $(this)
      .toggleClass("fa-sort-desc fa-sort-asc")
      .next()
      .toggleClass("mostra-menu-topo-mobile");
    //		var iconClick = $(this);
  ...
  $("#cx-usuario-topo").on(
    "click",
    "span.open-close, font.open-close",
    function (e) {
      e.preventDefault();
  ...
  $(document).on("click", "font.menus-site, #produtos-site", function (e) {
    ...
    if (
      $(document).find(".menus-lateral-pushindex").length === 0 &&
      window.location.pathname.split("/")[1].trim() === "produtos"
    ) {
      $.ajax({
        url: "/index",
        success: function (str) {
        ...
        complete: function () {
          $(document)
            .find(".menus-lateral")
            .addClass("menus-lateral-mobile menus-lateral-pushindex")
            .animate({ marginLeft: "0" }, 330)
            .attr({ visible: "true" });
        },
      });
    } else if ($(document).find(".menus-lateral").length === 0) {
      $.ajax({
        url: "/index",
        success: function (str) {
          var list = $("<div/>", { html: str });
