            ...
            $.ajax({
                ...
                complete: function() {
                    ...
                    if( $("#div-produto").find("#btn-comprar-mobile").length > 0 )
                        $("#div-produto").find("#btn-comprar-mobile").removeClass("btn-comprar-mobile");
                    if( $("#div-produto").find("#price-mobile").length > 0 )
                        $("#div-produto").find("#price-mobile").addClass("hidden");
                },
                success: function (a, b) {
                    var list = $("<div/>",{ html: a }),
                        ModalSite = $("#modal-site").modal("show").find(".modal-dialog").addClass("modal-lg");
                    if( b === 'success' ) {
...
(function($){
	...
	$.fn.extend({
		...
		caixa_cores: function ( options ) {
			...
			$(this).parents("a").attr({"href": $(this).parents("a").attr("href").replace(/(\d+)/g, settings.id) });

			$(this).parents("a").find("button[btn-comprar]").attr({"btn-comprar": $(this).parents("a").attr("href").replace(/(\d+)/g, settings.id) });

			$(this).parents('a').find('img').attr({'src': settings.src, 'data-original':  settings['data-original'] }).lazyload();
...
//    });
    ...
    $("input.input-pesquisar").focus(function () {
        ...
        $(".retornar-pesquisa").delay(550).queue(function(e){
            $(this).fadeOut(0);
            e();
        });
        ...
        $.ajax({
            ...
            dataType: "json",
            beforeSend: function() {
                $(".retornar-pesquisa").css({"border-width": "0px 1px 3px 1px", "min-height": "0"}).html("");
            },
            success: function(str) {
                        ...
                        $("<li/>", {
                            ...
                            html: [
                                $("<span/>", {
                                    ...
                                    html: $("<img/>", { src: b.image, class: "img-responsive" })
                                }),
                                $("<span/>", {
                                    ...
                                    html: [
                                        ...
                                        $("<span/>", { class: "show ft17px", html: b.preco_promo })
                                    ]
                                }),
                            ],
                            click: function(){
        ...
        if( test.find("i").length == 0 ) {
            test.append([
                $("<i/>", {
                    ...
                    css: { "z-index": "99", "width": "15px", "height": "15px", "line-height": "20px", "margin-top": (((h / 2) - (17 / 2)) * 1) },
                    click: function(e) {
                        test.next().fadeOut(110);
                        test.find("i").removeClass("fa-chevron-down").addClass("fa-chevron-right");
     ...
     */
    $(document).on("mouseenter", ".lista-menu-topo", function(){
        if ( $( window ).width() < 768 ) return false;

        $(this).stop().addClass("add-link-menu-topo").children(".lista-submenus-topo").fadeIn(110);

    }).on("mouseleave", ".lista-menu-topo", function() {
        if ( $( window ).width() < 768 ) return false;

        $(this).stop().delay(330).queue(function(ex) {
    ...
    $( window ).scroll(function () {
		if ( $( window ).width() > 768 && $IdentificacoHref < 0){
			if ( $( this ).scrollTop() >= $HeightTopo ){
				...
				$("#topo-movel").stop().addClass("add-topo-movel").fadeIn(900).find("#img-new-cart").fadeOut(0);
				$("body").css({"padding-top":$HeightTopo});
			} else {
				$("#topo-movel").removeAttr("style").removeClass("add-topo-movel").find("#img-new-cart").fadeIn();
